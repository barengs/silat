...
class ApprovalFlowSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $sppdSteps = [
            ...
            [
                'step_order' => 2,
                'step_label' => 'Verifikasi Administrasi Cabdin',
                'role_id_required' => $roleVerifikator?->id,
                'action_type' => 'verify',
            ],
            [
                'step_order' => 3,
                'step_label' => 'Persetujuan Akhir Dinas',
                'role_id_required' => $roleApprover?->id ?? $roleKadis?->id,
                'action_type' => 'approve',
            ],
        ];
        ...
        foreach ($sppdSteps as $step) {
            if ($step['role_id_required']) {
                ApprovalFlow::updateOrCreate(
                    ['module_name' => 'sppd', 'step_order' => $step['step_order']],
                    array_merge($step, ['is_active' => true])
                );
            }
        }
